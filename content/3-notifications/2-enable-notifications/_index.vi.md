...
2. Sử dụng lệnh ```aws backup put-backup-vault-notifications``` để kích hoạt tính năng Notification cho Backup Vault của bạn. Hãy thay đổi các thông số truyền vào của câu lệnh để phù hợp với tài nguyên của bạn.
    ```bash
    aws backup put-backup-vault-notifications --region ap-northeast-1 --backup-vault-name sharenote-backup-vault --backup-vault-events BACKUP_JOB_COMPLETED RESTORE_JOB_COMPLETED --sns-topic-arn arn:aws:sns:ap-northeast-1:xxx879421xxx:sharenote-backup-sns
    ```
